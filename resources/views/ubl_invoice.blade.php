...
<cbc:ProfileID>reporting:1.0</cbc:ProfileID>
    ...
    @foreach($order->products as $key=>$item)
        <cac:InvoiceLine>
            ...
            <cbc:LineExtensionAmount currencyID="{{ config('jo_fotara.currency_attribute') }}">
                {{ number_format((($item->pivot->quantity * number_format(($item->pivot->price / (1 + $order->tax_value)), 3, '.', '')) - number_format(($item->pivot->discount / (1+$order->tax_value)), 3, '.', '')), 3, '.', '')  }}</cbc:LineExtensionAmount>
            <cac:TaxTotal>
                <cbc:TaxAmount currencyID="{{ config('jo_fotara.currency_attribute') }}">
                    {{ number_format(((($item->pivot->quantity * number_format(($item->pivot->price / (1 + $order->tax_value)), 3, '.', '')) - number_format(($item->pivot->discount / (1+$order->tax_value)), 3, '.', '')) * $order->tax_value), 3, '.', '')}}</cbc:TaxAmount>
                <cbc:RoundingAmount currencyID="{{ config('jo_fotara.currency_attribute') }}">
                    {{ number_format((
                        (
                         (
                             $item->pivot->quantity * number_format(($item->pivot->price / (1 + $order->tax_value)), 3, '.', '')
                         )
                    - number_format(
                            ($item->pivot->discount / (1+$order->tax_value)), 3, '.', ''
                            )
                        )
                         * $order->tax_value
                         ) +  (
                             (
                                 $item->pivot->quantity * number_format(
                                     ($item->pivot->price / (1 + $order->tax_value)), 3, '.', '')
                                     ) - number_format(($item->pivot->discount / (1+$order->tax_value)), 3, '.', '')), 3, '.', '')  }}</cbc:RoundingAmount>
                <cac:TaxSubtotal>
                    <cbc:TaxAmount currencyID="{{ config('jo_fotara.currency_attribute') }}">
                        {{ number_format(((($item->pivot->quantity * number_format(($item->pivot->price / (1 + $order->tax_value)), 3, '.', '')) - number_format(($item->pivot->discount / (1+$order->tax_value)), 3, '.', '')) * $order->tax_value), 3, '.', '')}}</cbc:TaxAmount>
                    <cac:TaxCategory>
